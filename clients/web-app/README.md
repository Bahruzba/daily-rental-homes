...
Live rejimdə status, tarix və `hasExpiredDepositDeadline=true` filterləri `GET /api/broker/bookings` endpoint-inə query param kimi göndərilir. Response-da `hasPendingCancellationRequest` true olduqda kartda `Ləğv sorğusu`, `isDeadlineExpired` true olduqda isə `Beh vaxtı keçib` badge-i göstərilir. Mock rejimdə eyni filterlər lokal demo rezervasiyalar üzərində tətbiq olunur və bir demo rezervasiya ləğv sorğusu, başqa demo rezervasiya isə vaxtı keçmiş beh badge-i ilə göstərilir. Tarix filterində yalnız bir tarix seçilərsə və ya başlanğıc tarixi bitiş tarixindən sonra olarsa, frontend API çağırmadan oxunaqlı xəta göstərir. “Təmizlə” düyməsi filterləri, o cümlədən `Beh vaxtı keçənlər` seçimini sıfırlayır və bütün rezervasiyaları yenidən yükləyir.
