...
export function RentalHomeGrid({ homes, loading, onClear, isFavorite, onToggleFavorite, emptyTitle = 'Bu filtrə uyğun ev tapılmadı', emptyDescription = 'Başqa bölgə seçin və ya filtrləri təmizləyin.', emptyAction }: Props) {
  ...
  return <div className="homes-grid">{homes.map((home) => <RentalHomeCard key={home.id} home={home} isFavorite={isFavorite?.(home.id)} onToggleFavorite={onToggleFavorite} />)}</div>
}
