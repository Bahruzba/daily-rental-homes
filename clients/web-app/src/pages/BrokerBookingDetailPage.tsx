...
export function BrokerBookingDetailPage() {
  ...
  const [booking, setBooking] = useState<BrokerBookingDetail>(); const [loading, setLoading] = useState(true); const [saving, setSaving] = useState(false); const [error, setError] = useState(''); const [success, setSuccess] = useState('')
  ...
  const runAction = async (action: () => Promise<unknown>, successMessage: string) => { setSaving(true); setError(''); setSuccess(''); try { await action(); setSuccess(successMessage); await load() } catch (cause) { console.error('Broker deposit action failed', cause); setError(cause instanceof Error ? cause.message : 'Əməliyyat tamamlanmadı.') } finally { setSaving(false) } }
  const submitDeposit = (event: FormEvent) => { event.preventDefault(); if (!session) return; void runAction(() => requestBrokerDeposit(bookingId, { amount: Number(amount), deadlineAt: new Date(deadline).toISOString(), cardHolderName: holder, cardPanMasked: pan, bankName: bank, note: depositNote }, session.accessToken), 'Beh tələbi yaradıldı, müştəri bildirişi və reminder növbəyə alındı.') }
