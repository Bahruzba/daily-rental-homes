...
export function BrokerDashboardPage() {
  ...
  return <AppLayout><section className="broker-live-page"><div className="container">
    ...
    {loading ? <div className="broker-loading">Broker məlumatları yüklənir…</div> : <>
      ...
      <div className="broker-section-heading"><div><h2>Evlərim</h2><p>Broker hesabınıza bağlı elanlar.</p></div><Link className="button button-primary" to="/broker/rental-homes/new">Ev əlavə et</Link></div>
      {homes.length ? <div className="broker-homes-grid">{homes.map((home) => <Link to={`/broker/rental-homes/${home.id}/edit`} key={home.id}>{home.mainImageUrl ? <img src={home.mainImageUrl} alt={home.title} /> : <div className="broker-home-placeholder"><Building2 /></div>}<div><span>{home.city}{home.district ? ` · ${home.district}` : ''}</span><h3>{home.title}</h3><p>{home.guestCount} qonaq · {home.bookingCount} rezervasiya</p><strong>{money.format(home.dailyPrice)} / gecə</strong></div><em className={home.isPublished ? 'is-active' : ''}>{home.isPublished ? 'Aktiv' : 'Gizli'}</em></Link>)}</div> : <EmptyState title="Brokerə bağlı ev yoxdur" description="Ev əlavə edildikdə burada görünəcək." />}
